...
\midrule
Unitaria & JUnit 5 / Vitest / JUnit Android & ¿Esta clase, aislada, hace lo que dice? & 310/310 backend (5 servicios Java + Node + Python), 128/128 web, 33/33 móvil \\
...
para los cinco servicios Java (63, 155, 37, 8 y 16 pruebas respectivamente); \texttt{npm test} (que
invoca \texttt{node --test}) dentro de \texttt{services/notification-service} (15); \texttt{pytest}
dentro de \texttt{services/ai-service} (16) — total backend, 310; \texttt{npm test} (que invoca
...
La fila «Unitaria» no es una población disjunta de las filas «Integración» y «Contrato»: los 310
de backend incluyen las 2 pruebas de \texttt{TicketRepositoryIntegrationTest} (Testcontainers, la
misma clase que reporta la fila «Integración», porque \texttt{mvn test} las ejecuta en el mismo
módulo) y \emph{no} incluyen la prueba de proveedor Pact de \texttt{svc-principal}, que
\texttt{mvn test} omite con \texttt{@EnabledIfSystemProperty} salvo que se invoque explícitamente
(\texttt{-Dpact.verifier.publishResults=true}), y que la fila «Contrato» sí cuenta por separado
al correrla así. Los 128 de web incluyen las 2 pruebas de consumidor Pact
(\texttt{apps/web/tests/contract/ticketsApi.pact.test.ts}), que la fila «Contrato» vuelve a
publicar como «2/2 consumidor». Se documenta así, en vez de depurar cada fila para que sea
disjunta de las demás, porque las cifras de «Unitaria» son literalmente lo que produce
\texttt{mvn test}/\texttt{npm test} sin banderas adicionales —el mismo comando que corre la CI—, y
alterar esa cifra para excluir pruebas que técnicamente conviven en el mismo módulo dejaría de
corresponder a la salida real del comando publicado.
