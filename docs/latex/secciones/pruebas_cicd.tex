...
\texttt{report-service}, incluida la exportación a CSV. Cada una de las dieciocho declara un
esquema de respuesta exitosa tipado -- ninguna se queda en la envoltura genérica
\texttt{ApiResponse} sin decir qué va en \texttt{data} --, los códigos de estado posibles, y el
esquema de seguridad (\texttt{bearerAuth}, JWT) cuando aplica; se valida automáticamente en el
...
se agregue después. Una revisión externa mutó el contrato dos veces para verificar que la regla
anterior (una sola, \texttt{content: truthy}) en realidad no protegía nada: quitar \texttt{content}
de una operación sí rompía la construcción, pero cambiarlo por \texttt{application/json: \{\}} o
por \texttt{text/plain: \{\}} —un tipo de contenido declarado, sin ningún esquema— pasaba igual.
Las dos reglas actuales sí detectan ambas mutaciones (verificado reproduciendo el mismo ataque
contra el contrato corregido). \texttt{build-images} y \texttt{build-mobile-apk} dependen de
