...
\midrule
Unitaria & JUnit 5 / Vitest / JUnit Android & ¿Esta clase, aislada, hace lo que dice? & 183/183 backend (5 servicios Java + Node + Python), 75/75 web, 5/5 móvil \\
...
para los cinco servicios Java (26, 106, 23, 8 y 8 pruebas respectivamente); \texttt{npm test} (que
invoca \texttt{node --test}) dentro de \texttt{services/notification-service} (4); \texttt{pytest}
dentro de \texttt{services/ai-service} (8) — total backend, 183; \texttt{npm test} (que invoca
\texttt{vitest run}) dentro de \texttt{apps/web} (75); y \texttt{./gradlew
:app:testDebugUnitTest} dentro de \texttt{apps/mobile} para la capa unitaria móvil (5). Son los
