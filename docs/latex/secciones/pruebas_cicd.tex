...
\midrule
Unitaria & JUnit 5 / Vitest / JUnit Android & ¿Esta clase, aislada, hace lo que dice? & 319/319 backend (5 servicios Java + Node + Python), 126/126 web, 33/33 móvil \\
Integración & Testcontainers & ¿Funciona contra un CockroachDB real, no un \emph{mock}? & 6/6: \texttt{TicketRepositoryIntegrationTest} (svc-principal) + \texttt{FlywayMigrationIntegrationTest} (auth-service, report-service) \\
...
Cada cifra de la fila «Unitaria» se obtiene ejecutando \texttt{mvn clean test} —con
\texttt{clean}: un \texttt{mvn test} sobre un \texttt{target/} ya compilado en una corrida
anterior puede arrastrar \emph{surefire-reports} de clases que ya no existen o quedar a medio
recompilar, y una revisión propia posterior encontró exactamente ese error en una cifra
publicada antes en esta misma sección (167 en vez de las 156 reales de \texttt{svc-principal};
se corrige aquí)— y restando las pruebas que otra fila de esta misma tabla ya cuenta por
separado, para que las tres filas sean poblaciones disjuntas y la suma de la pirámide no
duplique nada: dentro de \texttt{services/auth-service}, \texttt{services/svc-principal},
\texttt{services/report-service}, \texttt{services/api-gateway} y
\texttt{services/telemetry-service} para los cinco servicios Java da 67, 165, 39, 8 y 16 pruebas
respectivamente (295 en total; \texttt{auth-service} sube de 65 a 67 con la prueba de
arquitectura \texttt{DomainArchitectureTest} que cierra su propia violación de capas, ver
Sección~\ref{sec:arquitectura-e4}, y \texttt{svc-principal} sube de 156 a 165: 8 pruebas nuevas en
\texttt{UpdateStatusRequestValidationTest} —una revisión externa encontró que las cinco
anotaciones \texttt{@Size}/\texttt{@DecimalMin}/\texttt{@DecimalMax} de \texttt{UpdateStatusRequest}
se podían borrar sin romper ninguna prueba, porque \texttt{TicketControllerTest} llama al
controlador directamente, sin \texttt{MockMvc} ni contexto de Spring, así que nunca dispara
\texttt{@Valid}; las 8 nuevas validan el \emph{record} con el mismo \texttt{Validator} de
\texttt{hibernate-validator} que usa Spring en producción, y reproducidamente fallan 5 de las 8 si
se borra cualquiera de las cinco anotaciones— más 1 prueba nueva en
\texttt{AuthGatewayFilterTest} que verifica que la línea de acceso lleva el mismo
\texttt{trace\_id} que el \texttt{MDC} de la petición, cierre de otro hallazgo de la misma
revisión (Sección~\ref{sec:movil} afirmaba esa correlación como ``verificada'' sin que ninguna
prueba la comprobara); de las 165 de \texttt{svc-principal} se restan las 2 de
\texttt{TicketRepositoryIntegrationTest} (Testcontainers, ya contadas en la fila «Integración») y
la 1 de \texttt{TicketServiceProviderPactTest} (ya contada en la fila «Contrato», omitida salvo con
\texttt{-DRUN\_CONTRACT\_VERIFICATION=true} y \emph{no} unitaria: verifica el contrato contra el
servicio real), y de las 67+39 de auth+report se restan las 2+2 de
\texttt{FlywayMigrationIntegrationTest} (mismo motivo) — 295 $-$ 2 $-$ 1 $-$ 2 $-$ 2 $=$
288, dejando 288 pruebas Java puramente unitarias; \texttt{npm test} dentro de
\texttt{services/notification-service} añade 15 y \texttt{pytest} dentro de
\texttt{services/ai-service} añade 16 (ninguna de las dos se solapa con otra fila) — total
backend, 319. \texttt{npm test} (\texttt{vitest run}) dentro de \texttt{apps/web} da 128, de las
que se restan las 2 de consumidor Pact
(\texttt{apps/web/tests/contract/ticketsApi.pact.test.ts}, ya contadas en «Contrato»), quedando
126. \texttt{./gradlew :app:testDebugUnitTest} dentro de \texttt{apps/mobile} da 33, sin
solapamiento. Los comandos son los mismos, sin variación, que ejecutan
\texttt{test-backend}/\texttt{test-web}/\texttt{test-mobile} de la Tabla~\ref{tab:cicd}; la resta
se documenta aquí mismo, con cada cifra bruta y cada descuento explícito, en vez de publicar el
número crudo del comando y dejar que el lector adivine qué se solapa.

Sumando las tres filas ya disjuntas: 319 (Unitaria backend) + 126 (Unitaria web) + 33 (Unitaria
móvil) + 6 (Integración: \texttt{TicketRepositoryIntegrationTest} + las dos
\texttt{FlywayMigrationIntegrationTest}) + 4 (Contrato: 2 consumidor + 2 proveedor —
\texttt{TicketServiceProviderPactTest} de \texttt{svc-principal}, omitido por \texttt{mvn test}
salvo con \texttt{-DRUN\_CONTRACT\_VERIFICATION=true}, ya restado arriba de la cifra de
\texttt{svc-principal} precisamente para que esta fila y la de Unitaria no lo cuenten las dos) da
\textbf{488 pruebas automatizadas distintas} en las tres capas inferiores de la pirámide, sin
ninguna contada dos veces.
...
por HTTP en vez de verificar la firma localmente (Sección~\ref{sec:contrato-openapi}). Sus 14
pruebas corren contra un doble real de \texttt{auth-service} (un
...
falla cerrado (401), nunca deja pasar; una de ellas verifica específicamente que la línea de
acceso lleva el mismo \texttt{trace\_id} que el \texttt{MDC} de la petición —una revisión externa
encontró que ninguna prueba anterior lo comprobaba, pese a que Sección~\ref{sec:movil} ya
afirmaba esa correlación como verificada. \texttt{HttpMetricsFilter} —la misma clase, copiada sin
...
\texttt{logger.js} (\texttt{notification-service}) no tenía ninguna prueba: es la
implementación propia del logging estructurado en JSON exigido por la Práctica Experimental U5
(ítem 2a, \emph{"logging estructurado (JSON) en todos los microservicios"}) —deliberadamente sin
dependencia nueva (nada de \texttt{winston}/\texttt{pino}), así que ninguna librería externa ya
probada por terceros garantiza que la línea impresa sea JSON válido con los campos correctos; esa
garantía solo la puede dar una prueba propia. Sus 4 pruebas nuevas confirman que
\texttt{info()}/\texttt{error()} escriben en la salida correcta (\texttt{console.log} vs.
\texttt{console.error}, la única rama real del archivo), que los campos \texttt{extra} se
mezclan en la línea, y que sin ellos no queda un \texttt{"undefined"} colado en el JSON
resultante.
...
solo en prosa, las dieciocho operaciones REST de los cinco microservicios: registro, login,
refresco, logout, validación de sesión y administración de cuentas (\texttt{auth-service}, siete
operaciones); creación, listado, detalle, cambio de estado, asignación e incidencias de tickets
(\texttt{ticket-service}, seis); notificaciones (\texttt{notification-service}, una);
clasificación automática (\texttt{ai-service}, una); y los tres puntos de agregación de
\texttt{report-service}, incluida la exportación a CSV. Diecisiete de las dieciocho declaran un
esquema de respuesta exitosa tipado -- no se quedan en la envoltura genérica
\texttt{ApiResponse} sin decir qué va en \texttt{data} --; la única excepción real y deliberada
es \texttt{POST /logout}, cuyo \texttt{data} es \texttt{null} siempre (revoca un \emph{refresh
token}, no devuelve ningún recurso), así que declarar un tipo para un campo que nunca lleva
contenido sería artificial -- se documenta aquí como excepción en vez de forzar un esquema sin
sentido o dejar la afirmación general sin matizar (hallazgo de una revisión externa). Todas
declaran los códigos de estado posibles y el esquema de seguridad (\texttt{bearerAuth}, JWT)
cuando aplica; se valida automáticamente en el
flujo con Spectral (Sección~\ref{sec:validacion-contrato}), que rompe la construcción si alguna
deja de cumplirlo. Cada ruta declara además su propio \texttt{servers:} (\emph{gateway} +
puerto directo del servicio real), en vez de heredar los diez \texttt{servers} globales sin
distinción -- antes de esto, un cliente generado a partir del contrato habría resuelto toda ruta
contra el primero de los diez (\texttt{auth}), sin importar a qué servicio perteneciera en
realidad.

Este archivo no es un anexo estático: documenta la misma API que ejercitan las pruebas de
contrato Pact de la siguiente subsección, y es lo que permite a un integrante nuevo entender la
API completa sin leer el código de los cinco servicios. Dicho esto, hoy son artefactos
independientes, no enlazados por herramienta: ningún archivo de \texttt{pacts/} ni de las
pruebas de \texttt{apps/web} referencia \texttt{openapi.yaml}, así que mantenerlos sincronizados
es por ahora una disciplina manual verificada en revisión, no una garantía mecánica -- se
declara así en vez de afirmar una referencia cruzada que el repositorio no respalda. Se
actualizó para esta entrega —seguía describiendo los puertos directos de la Entrega 2 como si
fueran la única vía de acceso, sin mencionar el API Gateway que desde la Entrega 4 es el punto de
entrada real de ambos clientes (Sección~\ref{sec:arquitectura-e4})— para reflejar las dos rutas
reales: el \emph{gateway} en el puerto 8000 como camino de producción, y los puertos directos de
cada servicio conservados solo como vía de depuración local.
...
profesor, y al revisar las diecisiete de entonces resultaron ser once, no cuatro. Para que ese
hueco no vuelva a pasar desapercibido, el trabajo \texttt{validate-openapi} del flujo
(\texttt{.github/workflows/ci-cd.yml}) corre \texttt{Spectral} (versión \texttt{6.16.3}, fijada
en el propio \texttt{ci-cd.yml} -- antes \texttt{npx --yes} sin versión quedaba a merced de lo
que \texttt{npm} resolviera como más reciente en cada corrida, sin garantía de evaluar el mismo
contrato de la misma forma mañana que hoy) sobre
\texttt{docs/api/openapi.yaml} en cada envío y cada \emph{pull request}, con dos reglas propias
declaradas en \texttt{.spectral.yaml}: \texttt{response-schema-required} exige que toda
respuesta 2xx declare al menos un tipo de contenido, y \texttt{response-content-schema-required}
exige que ese tipo de contenido tenga un \texttt{schema} no vacío -- no solo
\texttt{application/json}, para no repetir el mismo hueco en \texttt{text/csv} u otro tipo que
se agregue después. Una revisión externa mutó el contrato varias veces, en dos rondas, para verificar que las reglas
protegían de verdad. Primera ronda: la regla anterior (una sola, \texttt{content: truthy}) en
realidad no protegía nada -- quitar \texttt{content} de una operación sí rompía la construcción,
pero cambiarlo por \texttt{application/json: \{\}} o por \texttt{text/plain: \{\}} —un tipo de
contenido declarado, sin ningún esquema— pasaba igual; de ahí las dos reglas actuales.
Segunda ronda, sobre esas dos reglas ya corregidas: quitar \texttt{content} por completo de
\texttt{POST /logout} \emph{seguía} sin romper nada, porque el \texttt{given} de
\texttt{response-schema-required} apuntaba directo a \texttt{.content} -- si esa clave no existe,
el selector JSONPath no encuentra nada que evaluar, así que Spectral no reporta ninguna
violación ("nada que revisar" en vez de "falta algo obligatorio"); y \texttt{schema: \{\}} (un
objeto vacío, sin quitar la clave) tampoco rompía \texttt{response-content-schema-required},
porque \texttt{\{\}} es \emph{truthy} en JavaScript. Se corrigieron ambas: el \texttt{given} de
la primera regla ahora apunta al objeto de la respuesta 2xx en sí, no a \texttt{.content}, para
que su ausencia también caiga bajo la regla; y las dos reglas usan la función \texttt{schema} de
Spectral con \texttt{minProperties: 1} en vez de \texttt{truthy}/\texttt{length}, para que un
objeto vacío no pase como "presente". Las cinco mutaciones (quitar \texttt{content},
\texttt{application/json: \{\}}, \texttt{text/plain: \{\}}, \texttt{schema: \{\}}, y la
combinación de las dos rondas) fallan hoy contra el contrato corregido, reproducidas de nuevo
sobre esta misma versión antes de esta entrega. \texttt{build-images} y \texttt{build-mobile-apk}
dependen de
\texttt{validate-openapi} en su \texttt{needs}, así que un contrato roto ya no solo pone el
\emph{run} en rojo: impide también publicar las imágenes de \emph{Docker} y el APK/manuscrito de
la entrega.
...
El archivo \texttt{.github/workflows/ci-cd.yml} define diez \emph{jobs} en un grafo dirigido
...
\centering
\small
\caption{Los diez jobs del pipeline.}
\label{tab:cicd}
...
2 & \texttt{validate-openapi} & Spectral valida \texttt{docs/api/openapi.yaml} contra las dos reglas propias de \texttt{.spectral.yaml} \\
3 & \texttt{test-backend} & Los cinco servicios Java con pruebas automatizadas + Node + Python, pruebas unitarias e integración \\
4 & \texttt{test-web} & Vitest, incluido el contrato Pact del consumidor \\
5 & \texttt{test-mobile} & Unitarias JVM + instrumentadas en un emulador Android headless real \\
6 & \texttt{build-images} & Construye y publica las ocho imágenes Docker en GHCR (solo en \emph{push}) \\
7 & \texttt{build-mobile-apk} & Compila, firma y publica el APK de \emph{release} junto al manuscrito, bajo una etiqueta de versión que se mueve al commit actual en cada publicación \\
8 & \texttt{integration} & Levanta el \emph{stack} completo y corre Pact proveedor + Playwright contra el backend real \\
9 & \texttt{compile-latex} & Recompila el manuscrito y falla ante referencias/citas indefinidas o etiquetas duplicadas \\
10 & \texttt{verify-checksums} & Verifica los tres manifiestos SHA256SUMS versionados contra los datos crudos \\
\bottomrule
\end{tabular}
\end{table}

Los \emph{jobs} 6 y 7 declaran \texttt{needs} sobre los \emph{jobs} de prueba correspondientes:
...
--build}), no depende de las que publica GHCR. La declaración final de \texttt{build-images} es
\texttt{needs: [lint, test-backend, test-web, test-mobile, integration]}.

Una tercera revisión, esta vez sobre \texttt{build-mobile-apk} —el \emph{job} que firma el APK y
publica el \emph{release} de la entrega—, encontró la misma clase de hueco: su declaración
original era \texttt{needs: [test-mobile, compile-latex]} únicamente, así que el instalable
firmado y el manuscrito se publicaban aunque \texttt{lint}, \texttt{validate-openapi} o la suite
completa de \texttt{test-backend}/\texttt{test-web}/\texttt{integration} estuvieran en rojo. Se
corrigió a \texttt{needs: [lint, validate-openapi, test-backend, test-web, test-mobile,
integration, compile-latex]}; una revisión externa posterior encontró que a esa lista aún le
faltaba \texttt{verify-checksums} —la publicación podía salir con un manifiesto de datos crudos
roto sin que nada lo impidiera—, así que la declaración final son ocho \emph{jobs}:
\texttt{needs: [lint, validate-openapi, test-backend, test-web, test-mobile, integration,
compile-latex, verify-checksums]}.

Un segundo defecto de \texttt{build-mobile-apk}, independiente del anterior: la etiqueta bajo la
que se publicaba (\texttt{mobile-release}) no era una etiqueta de \emph{versión} en ningún
sentido real. \texttt{softprops/action-gh-release} reutiliza el \emph{ref} de git de un tag ya
existente en vez de moverlo al commit actual, así que aunque los binarios adjuntos se
reconstruían en cada \emph{push} a \texttt{main}, el tag de git seguía apuntando al commit donde
se creó por primera vez —etiqueta y contenido publicado dejaban de corresponder silenciosamente.
Se agregó un paso explícito (\texttt{git tag -f ... \&\& git push --force}) que mueve la etiqueta
\texttt{v1.0-entrega-final} al commit actual \emph{antes} de publicar, y se renombró la etiqueta
en consecuencia. Una revisión externa posterior encontró dos huecos más en este mismo mecanismo:
(a) al ser una etiqueta que se fuerza en cada \emph{push}, nunca hay una versión \emph{inmutable}
de una entrega concreta —se agregó una segunda etiqueta ligera por commit
(\texttt{entrega-<7 caracteres del SHA>}), creada sin \texttt{-f} junto a la móvil, que nunca se
mueve ni se sobrescribe; y (b) el flujo no serializaba ejecuciones solapadas de este \emph{job}
—dos \emph{pushes} seguidos podían entrelazar sus pasos de "mover etiqueta" y "publicar" en
cualquier orden—, corregido con \texttt{concurrency: \{group: build-mobile-apk-\$\{\{ github.ref
\}\}, cancel-in-progress: false\}}, que hace que el \emph{run} más nuevo espere a que el anterior
termine de publicar en vez de competir con él.
...
\midrule
Líneas & 705 & 729 & \textbf{96.7\,\%} \\
Instrucciones & 3461 & 3886 & 89.1\,\% \\
Ramas & 135 & 172 & 78.5\,\% \\
\bottomrule
\end{tabular}
\end{table}
...
exigente y la que ya superó ampliamente el objetivo del 70\,\% que el proyecto se propuso desde
la Entrega 3 (78.5\,\%, tras las rondas de pruebas de esta entrega). El paquete
\texttt{infrastructure.security} (filtro y configuración de JWT), que en una revisión externa
anterior a esta corrección tenía \textbf{0 de 38 líneas cubiertas} —ningún test ejercitaba las
ramas de token inválido/expirado/ausente directamente sobre esas clases, solo indirectamente vía
las pruebas de integración HTTP 401/403, que JaCoCo no le atribuye—, quedó cerrado con
\texttt{AuthGatewayFilterTest}; la clase \texttt{AuthGatewayFilter} creció después con la línea
de acceso real del entregable \#11 (de 38 a 40 líneas, las 40 cubiertas). El paquete completo
(\texttt{AuthGatewayFilter} + \texttt{ValidateResponse} + la clase interna anónima
\texttt{AuthGatewayFilter\$1}) suma \textbf{41 de 41} líneas cubiertas según el agregado por
paquete de \texttt{jacoco.xml} —no 42: el \texttt{jacoco.csv}, que reporta una fila por clase, da
40+1+1=42 porque atribuye a \texttt{AuthGatewayFilter\$1} la misma línea fuente donde se declara
dentro de \texttt{AuthGatewayFilter}, y el XML deduplica esa línea compartida al agregarla por
paquete en vez de sumarla dos veces. El paquete que más arrastra la
cifra de ramas hacia abajo hoy es \texttt{domain}, y en concreto una sola clase:
\texttt{TicketId} (0 de 22 ramas cubiertas). No es una omisión: \texttt{TicketId} es la clave
compuesta \texttt{@IdClass} de \texttt{Ticket} (\texttt{createdAt}, \texttt{id}), generada por
Lombok (\texttt{@Data}) para \texttt{equals()}/\texttt{hashCode()}; igual que el código generado
por \texttt{protobuf} más abajo en esta sección, ninguna convención de pruebas exige ejercitar a
mano cada rama de un \texttt{equals} mecánico sobre dos campos, y el propio JPA nunca invoca esa
clave compuesta en tiempo de ejecución —el punto de acceso real es
\texttt{TicketRepository.findByTicketId(UUID)}, como documenta el comentario de la clase—, así
que se declara aquí como brecha conocida y deliberadamente no priorizada, no oculta.

El reporte XML de JaCoCo agrega, a nivel de \texttt{<report>}, un total de 700/724 líneas —no
las 705/729 que resultan de sumar el CSV fila por fila. La diferencia no es un error de cálculo:
...
otra herramienta. Es la tercera generación de ese directorio: la segunda quedó fechada al
commit \texttt{31f1ef5} (87 pruebas) y no se regeneró cuando entregables posteriores añadieron
código y pruebas, así que las cifras publicadas describían un estado anterior aunque el propio
directorio se presentara como evidencia viva del análisis; se regenera aquí junto con esta
misma corrección para que texto y evidencia vuelvan a coincidir.
...
de líneas sin cubrir — no una elección arbitraria — y se agregaron 31 pruebas unitarias nuevas en
esa ronda inicial (la cifra total del módulo, acumulada tras todas las rondas posteriores, es la
que reporta la Tabla~\ref{tab:piramide}, no un número fijo en este párrafo) sobre, entre otras:
\texttt{CorrelationService} (el
