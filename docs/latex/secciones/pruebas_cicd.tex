...
\midrule
Unitaria & JUnit 5 / Vitest / JUnit Android & ¿Esta clase, aislada, hace lo que dice? & 326/326 backend (5 servicios Java + Node + Python), 128/128 web, 33/33 móvil \\
...
para los cinco servicios Java (65, 167, 39, 8 y 16 pruebas respectivamente); \texttt{npm test} (que
invoca \texttt{node --test}) dentro de \texttt{services/notification-service} (15); \texttt{pytest}
dentro de \texttt{services/ai-service} (16) — total backend, 326; \texttt{npm test} (que invoca
...
La fila «Unitaria» no es una población disjunta de las filas «Integración» y «Contrato»: los 326
de backend incluyen las 6 pruebas de Testcontainers —\texttt{TicketRepositoryIntegrationTest}
(svc-principal, 2) y \texttt{FlywayMigrationIntegrationTest} (auth-service y report-service, 2
cada uno; Entregable 5 de la guía de cierre: antes ninguna prueba de esos dos servicios aplicaba
su migración Flyway contra un CockroachDB real, solo el arranque del stack completo en
\texttt{integration})—, las mismas clases que reporta la fila «Integración» porque
\texttt{mvn test} las ejecuta en el mismo módulo, y \emph{no} incluyen la prueba de proveedor
Pact de \texttt{svc-principal}, que \texttt{mvn test} omite con \texttt{@EnabledIfSystemProperty}
salvo que se invoque explícitamente (\texttt{-Dpact.verifier.publishResults=true}), y que la fila
«Contrato» sí cuenta por separado al correrla así. Los 128 de web incluyen las 2 pruebas de
consumidor Pact (\texttt{apps/web/tests/contract/ticketsApi.pact.test.ts}), que la fila
«Contrato» vuelve a publicar como «2/2 consumidor». Se documenta así, en vez de depurar cada fila
para que sea disjunta de las demás, porque las cifras de «Unitaria» son literalmente lo que
produce \texttt{mvn test}/\texttt{npm test} sin banderas adicionales —el mismo comando que corre
la CI—, y alterar esa cifra para excluir pruebas que técnicamente conviven en el mismo módulo
dejaría de corresponder a la salida real del comando publicado. Para quien quiera un total sin
solapamiento entre filas: \texttt{Unitaria} sin las 6 de Testcontainers ni las 2 de Pact
consumidor son 320~backend + 126~web + 33~móvil = 479; sumando las 4 de \texttt{Contrato} (2
consumidor + 2 proveedor, este último no incluido en ningún otro conteo) y las 6 de
\texttt{Integración} (las mismas 6 que ya se restaron de \texttt{Unitaria}, no se vuelven a
sumar) da 479 + 4 + 6 = 489 pruebas automatizadas distintas en las tres capas inferiores de la
pirámide.
