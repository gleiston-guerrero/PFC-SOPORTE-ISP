...
\midrule
Unitaria & JUnit 5 / Vitest / JUnit Android & ¿Esta clase, aislada, hace lo que dice? & 320/320 backend (5 servicios Java + Node + Python), 126/126 web, 33/33 móvil \\
...
Cada cifra de la fila «Unitaria» se obtiene ejecutando la suite correspondiente y restando las
pruebas que otra fila de esta misma tabla ya cuenta por separado, para que las tres filas sean
poblaciones disjuntas y la suma de la pirámide no duplique nada: \texttt{mvn test} dentro de
\texttt{services/auth-service}, \texttt{services/svc-principal}, \texttt{services/report-service},
\texttt{services/api-gateway} y \texttt{services/telemetry-service} para los cinco servicios Java
da 65, 167, 39, 8 y 16 pruebas respectivamente (325 en total); de las 167 de \texttt{svc-principal}
se restan las 2 de \texttt{TicketRepositoryIntegrationTest} (Testcontainers, ya contadas en la
fila «Integración») y de las 65+39 de auth+report se restan las 2+2 de
\texttt{FlywayMigrationIntegrationTest} (mismo motivo), dejando 320 pruebas Java puramente
unitarias; \texttt{npm test} dentro de \texttt{services/notification-service} añade 15 y
\texttt{pytest} dentro de \texttt{services/ai-service} añade 16 (ninguna de las dos se solapa con
otra fila) — total backend, 320. \texttt{npm test} (\texttt{vitest run}) dentro de
\texttt{apps/web} da 128, de las que se restan las 2 de consumidor Pact
...
Sumando las tres filas ya disjuntas: 320 (Unitaria backend) + 126 (Unitaria web) + 33 (Unitaria
...
no se restó de ninguna cifra de \texttt{svc-principal}) da \textbf{489 pruebas automatizadas
