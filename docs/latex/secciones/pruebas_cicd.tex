...
\midrule
Unitaria & JUnit 5 / Vitest / JUnit Android & ¿Esta clase, aislada, hace lo que dice? & 309/309 backend (5 servicios Java + Node + Python), 126/126 web, 33/33 móvil \\
...
de las 65+39 de auth+report se restan las 2+2 de \texttt{FlywayMigrationIntegrationTest} (mismo
motivo), dejando 309 pruebas Java puramente unitarias; \texttt{npm test} dentro de
...
backend, 309. \texttt{npm test} (\texttt{vitest run}) dentro de \texttt{apps/web} da 128, de las
...
Sumando las tres filas ya disjuntas: 309 (Unitaria backend) + 126 (Unitaria web) + 33 (Unitaria
móvil) + 6 (Integración: \texttt{TicketRepositoryIntegrationTest} + las dos
\texttt{FlywayMigrationIntegrationTest}) + 4 (Contrato: 2 consumidor + 2 proveedor, este último
omitido por \texttt{mvn test} salvo con \texttt{-Dpact.verifier.publishResults=true} y que por eso
no se restó de ninguna cifra de \texttt{svc-principal}) da \textbf{478 pruebas automatizadas
distintas} en las tres capas inferiores de la pirámide, sin ninguna contada dos veces.
