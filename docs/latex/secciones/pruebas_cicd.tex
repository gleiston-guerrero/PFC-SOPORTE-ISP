...
\midrule
Unitaria & JUnit 5 / Vitest / JUnit Android & ¿Esta clase, aislada, hace lo que dice? & 250/250 backend (5 servicios Java + Node + Python), 94/94 web, 25/25 móvil \\
...
para los cinco servicios Java (42, 123, 35, 8 y 15 pruebas respectivamente); \texttt{npm test} (que
invoca \texttt{node --test}) dentro de \texttt{services/notification-service} (11); \texttt{pytest}
dentro de \texttt{services/ai-service} (16) — total backend, 250; \texttt{npm test} (que invoca
\texttt{vitest run}) dentro de \texttt{apps/web} (94); y \texttt{./gradlew
:app:testDebugUnitTest} dentro de \texttt{apps/mobile} para la capa unitaria móvil (25). Son los
