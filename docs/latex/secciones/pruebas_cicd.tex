...
\midrule
Unitaria & JUnit 5 / Vitest / JUnit Android & ¿Esta clase, aislada, hace lo que dice? & 310/310 backend (5 servicios Java + Node + Python), 126/126 web, 33/33 móvil \\
...
\texttt{services/telemetry-service} para los cinco servicios Java da 67, 156, 39, 8 y 16 pruebas
respectivamente (286 en total; \texttt{auth-service} sube de 65 a 67 con la prueba de
arquitectura \texttt{DomainArchitectureTest} que cierra su propia violación de capas, ver
Sección~\ref{sec:arquitectura-e4}); de las 156 de \texttt{svc-principal} se restan las 2 de
...
\texttt{-Dpact.verifier.publishResults=true} y \emph{no} unitaria: verifica el contrato contra el
servicio real), y de las 67+39 de auth+report se restan las 2+2 de
\texttt{FlywayMigrationIntegrationTest} (mismo motivo) — 286 $-$ 2 $-$ 1 $-$ 2 $-$ 2 $=$ 279,
dejando 279 pruebas Java puramente unitarias; \texttt{npm test} dentro de
...
backend, 310. \texttt{npm test} (\texttt{vitest run}) dentro de \texttt{apps/web} da 128, de las
...
Sumando las tres filas ya disjuntas: 310 (Unitaria backend) + 126 (Unitaria web) + 33 (Unitaria
...
salvo con \texttt{-Dpact.verifier.publishResults=true}, ya restado arriba de la cifra de
\texttt{svc-principal} precisamente para que esta fila y la de Unitaria no lo cuenten las dos) da
\textbf{479 pruebas automatizadas distintas} en las tres capas inferiores de la pirámide, sin
