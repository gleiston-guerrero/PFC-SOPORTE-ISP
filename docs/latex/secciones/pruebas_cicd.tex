...
\midrule
Unitaria & JUnit 5 / Vitest / JUnit Android & ¿Esta clase, aislada, hace lo que dice? & 200/200 backend (5 servicios Java + Node + Python), 94/94 web, 20/20 móvil \\
...
invoca \texttt{node --test}) dentro de \texttt{services/notification-service} (7); \texttt{pytest}
dentro de \texttt{services/ai-service} (12) — total backend, 200; \texttt{npm test} (que invoca
\texttt{vitest run}) dentro de \texttt{apps/web} (94); y \texttt{./gradlew
:app:testDebugUnitTest} dentro de \texttt{apps/mobile} para la capa unitaria móvil (20). Son los
