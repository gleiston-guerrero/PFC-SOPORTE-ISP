...
\midrule
Unitaria & JUnit 5 / Vitest / JUnit Android & ¿Esta clase, aislada, hace lo que dice? & 318/318 backend (5 servicios Java + Node + Python), 126/126 web, 33/33 móvil \\
...
\texttt{services/telemetry-service} para los cinco servicios Java da 67, 164, 39, 8 y 16 pruebas
respectivamente (294 en total; \texttt{auth-service} sube de 65 a 67 con la prueba de
arquitectura \texttt{DomainArchitectureTest} que cierra su propia violación de capas, ver
Sección~\ref{sec:arquitectura-e4}, y \texttt{svc-principal} sube de 156 a 164 con
...
se podían borrar sin romper ninguna prueba —\texttt{TicketControllerTest} llama al controlador
directamente, sin \texttt{MockMvc} ni contexto de Spring, así que nunca dispara \texttt{@Valid}—;
las 8 pruebas nuevas validan el \emph{record} con el mismo \texttt{Validator} de
\texttt{hibernate-validator} que usa Spring en producción, sin contexto Spring, y reproducidamente
fallan 5 de las 8 si se borra cualquiera de las cinco anotaciones); de las 164 de
\texttt{svc-principal} se restan las 2 de \texttt{TicketRepositoryIntegrationTest} (Testcontainers,
ya contadas en la fila «Integración») y la 1 de \texttt{TicketServiceProviderPactTest} (ya contada
en la fila «Contrato», omitida salvo con \texttt{-DRUN\_CONTRACT\_VERIFICATION=true} y \emph{no}
unitaria: verifica el contrato contra el servicio real), y de las 67+39 de auth+report se restan
las 2+2 de \texttt{FlywayMigrationIntegrationTest} (mismo motivo) — 294 $-$ 2 $-$ 1 $-$ 2 $-$ 2 $=$
287, dejando 287 pruebas Java puramente unitarias; \texttt{npm test} dentro de
...
backend, 318. \texttt{npm test} (\texttt{vitest run}) dentro de \texttt{apps/web} da 128, de las
...
Sumando las tres filas ya disjuntas: 318 (Unitaria backend) + 126 (Unitaria web) + 33 (Unitaria
...
\textbf{487 pruebas automatizadas distintas} en las tres capas inferiores de la pirámide, sin
