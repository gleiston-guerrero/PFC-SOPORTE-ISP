...
\caption{C4 Nivel 2 — contenedores del sistema, Entrega 4. \emph{Nota de vigencia}: una revisión
propia posterior a esta figura encontró dos imprecisiones en su fuente Mermaid, ya corregidas en
\texttt{docs/diagrams/c4-nivel2-contenedores.md} pero pendientes de re-exportar a esta imagen (sin
renderizador Mermaid local que reproduzca la calidad de mermaid.live para este diagrama, igual que
la limitación ya documentada para el Nivel 3): el rótulo de Prometheus decía «x6» donde el conteo
real, verificado contra \texttt{ops/prometheus/prometheus.yml}, es 4; y faltaban las relaciones
\texttt{api-gateway}/\texttt{telemetry-service} $\to$ \texttt{otel-collector}, que sí exportan
trazas en el código actual.}
\label{fig:c4-nivel2}
\end{figure}
