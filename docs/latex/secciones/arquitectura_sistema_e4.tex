...
\caption{C4 Nivel 2 — contenedores del sistema, Entrega 4. \emph{Nota de vigencia}: revisiones
propias posteriores a esta figura encontraron cuatro imprecisiones en su fuente Mermaid, ya
corregidas en \texttt{docs/diagrams/c4-nivel2-contenedores.md} pero pendientes de re-exportar a
esta imagen (sin renderizador Mermaid local que reproduzca la calidad de mermaid.live para este
diagrama en concreto —probado dos veces con \texttt{mermaid-cli}, con y sin acortar etiquetas,
mismo resultado ilegible—, igual que la limitación ya documentada para el Nivel 3): el rótulo de
Prometheus decía «x6» donde el conteo real, verificado contra
\texttt{ops/prometheus/prometheus.yml}, es 4; faltaban las relaciones
\texttt{api-gateway}/\texttt{telemetry-service} $\to$ \texttt{otel-collector}, que sí exportan
trazas en el código actual; y la relación Prometheus/collector decía \texttt{remote\_write}
(push) cuando en realidad es un \emph{scrape} (Prometheus extrae, no el collector empuja —
\texttt{ops/otel-collector/config.yaml} no declara ningún exportador \texttt{prometheusremotewrite}).}
\label{fig:c4-nivel2}
\end{figure}
...
\texttt{infrastructure}/\texttt{presentation}), con una única excepción \emph{ejercida} en el
código: \texttt{infrastructure} depende de \texttt{presentation} porque \texttt{AuthGatewayFilter}
reutiliza el sobre de respuesta \texttt{ApiResponse} para las respuestas 401/403 que rechaza antes
de que la petición llegue a un controlador. La propia regla de \texttt{Architectures}, tal como
está escrita (\texttt{whereLayer("Infrastructure").mayOnlyBeAccessedByLayers("Presentation")}),
también permite en abstracto la dirección contraria —\texttt{presentation} dependiendo de
\texttt{infrastructure}, el sentido habitual de inyección de dependencias en cualquier capa de
presentación—, pero hoy ningún controlador de ningún servicio la ejerce; una revisión externa
señaló, con razón, que llamar a esto ``la única excepción'' sin matizar sugiere que ArchUnit
prohíbe esa segunda dirección, cuando en realidad solo no está en uso.
...
\texttt{auth-service} & 4 capas + \texttt{DomainArchitectureTest} propio & \texttt{User}/\texttt{RefreshToken} eran entidades JPA
  directamente en el paquete \texttt{domain}; \texttt{AuthService} dependía de
  \texttt{JpaRepository} sin un puerto intermedio. Corrección posterior (ver más abajo):
  \texttt{AuthService} también dependía directamente de
  \texttt{infrastructure.security.JwtService},
  \texttt{infrastructure.messaging.TechnicianEventPublisher} y cinco DTO de
  \texttt{presentation.dto} — 38 violaciones reales que ninguna prueba detectaba porque este
  servicio nunca tuvo su propio \texttt{DomainArchitectureTest} \\
...
Una revisión externa posterior aplicó a \texttt{auth-service} la misma regla
\texttt{Architectures.layeredArchitecture()} que \texttt{ticket-service} ya tenía y encontró 38
violaciones reales en \texttt{AuthService.java}: la clase de \texttt{application} importaba
directamente \texttt{infrastructure.security.JwtService},
\texttt{infrastructure.messaging.TechnicianEventPublisher} y los DTO de
\texttt{presentation.dto} (\texttt{AuthResponse}, \texttt{UserResponse},
\texttt{ValidateResponse}, \texttt{RegisterRequest}, \texttt{CreateUserRequest}) --
\texttt{auth-service} nunca tuvo su propio \texttt{DomainArchitectureTest} que lo detectara, así
que la afirmación anterior de "el mismo patrón" no estaba verificada, solo declarada. Se corrige
con el mismo mecanismo de inversión de dependencias que \texttt{ticket-service}:
\texttt{application/TokenIssuer.java} y \texttt{application/TechnicianCreatedNotifier.java} son
los puertos que \texttt{AuthService} conoce ahora (\texttt{JwtService} y
\texttt{TechnicianEventPublisher}, en \texttt{infrastructure}, los implementan); y
\texttt{AuthService} ya no recibe ni devuelve DTO de \texttt{presentation} --recibe
\texttt{RegisterCommand}/\texttt{CreateUserCommand} y devuelve
\texttt{User}/\texttt{TokenPair}/\texttt{TokenValidation}, todos de \texttt{application}--, y es
\texttt{AuthController}/\texttt{AdminUserController} quien mapea hacia/desde los DTO HTTP, mismo
patrón que \texttt{TicketController} mapea \texttt{Ticket} a \texttt{TicketResponse}. Se agregó
\texttt{auth-service/architecture/DomainArchitectureTest.java}, idéntico en estructura al de
\texttt{ticket-service} (misma excepción declarada: \texttt{infrastructure} puede depender de
\texttt{presentation} porque \texttt{SecurityConfig} reutiliza \texttt{ApiResponse} para sus
respuestas 401/403), que pasa en verde (0 violaciones) sobre el código corregido.
