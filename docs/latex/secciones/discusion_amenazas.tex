...
media de 1.8, 477 pruebas automatizadas distintas ejecutadas en el último ciclo entre las capas
Unitaria+Integración+Contrato de la pirámide de pruebas (308 backend + 126 web + 33 móvil
unitarias, sin solapamiento entre filas, más 6 de Integración y 4 de Contrato), ver la
Tabla~\ref{tab:piramide} de la Sección~\ref{sec:pruebas-cicd}, donde consta el comando de cada
suite --- y las relacionadas con \textbf{compatibilidad de cliente} (la misma suite E2E pasa en tres
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Datos de demostración generados sintéticamente}: los tickets de prueba (150\,000
        filas, distribuidas cada 210 segundos a lo largo de 2026 para cubrir las cuatro
        particiones trimestrales, \texttt{generate\_series(1, 150000)}) no provienen de un ISP
        real —mitigado documentando explícitamente el script de generación
        (\texttt{db-cluster/scripts/seed\_partitioned.sql}, o su variante en lotes
        \texttt{seed\_partitioned\_batched.sql} para evitar el OOM de una única transacción de
        150\,000 filas) para que la distribución de estados sea auditable y reproducible. Una
        revisión propia posterior a esta entrega encontró que una versión anterior de este mismo
        párrafo citaba una cifra (504+) y una ruta (\texttt{resultados/rebalance\_demo\_tickets.sql})
        que no corresponden a ningún archivo del repositorio ni a ninguna corrida real —se
        corrige aquí a la cifra y la ruta que sí se pueden reproducir ejecutando el guion citado.
\end{enumerate}
