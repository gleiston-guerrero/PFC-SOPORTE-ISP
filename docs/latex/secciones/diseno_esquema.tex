...
\subsection{Evolución del esquema: cierre en sitio (\texttt{V3})}

El esquema de \texttt{tickets} no quedó congelado en \texttt{V1}. La capacidad de cierre en sitio
del entregable 10 de la guía de cierre (Sección~\ref{sec:movil}) requirió tres columnas
adicionales, añadidas por una tercera migración Flyway posterior,
\texttt{V3\_\_add\_close\_evidence.sql}, que corre después de \texttt{V1} y \texttt{V2} sin
modificar ninguna de las dos:

\begin{lstlisting}[language=SQL, caption={Columnas de evidencia del cierre en sitio (\texttt{V3\_\_add\_close\_evidence.sql})}, label={lst:v3-evidence}]
...
Las tres son nulables por diseño: solo el cierre por un técnico vía la app móvil las llena (con la
foto codificada en Base64 sobre el transporte HTTP y decodificada a \texttt{BYTES} en el servidor,
Sección~\ref{sec:movil}); un \texttt{ADMIN} puede seguir resolviendo un ticket sin pasar
por el móvil, y el resto de las transiciones de estado no las toca. No participan de la
fragmentación ni de la política de replicación de las secciones anteriores —ninguna es la columna
de partición ni cambia el factor de réplicas—, así que \texttt{V3} extiende el esquema físico sin
alterar ninguna de las tres propiedades de corrección verificadas más arriba para \texttt{V1}, ni
la política de \texttt{V2}. Reconstruir el esquema completo de \texttt{ticket\_db} desde cero es,
por tanto, aplicar las tres migraciones en orden (\texttt{V1} → \texttt{V2} → \texttt{V3}) contra
una base vacía —el procedimiento exacto, verificado contra un clúster real, está en
\texttt{db-cluster/README.md}—, no una migración única ni un volcado de esquema independiente.

El propio archivo \texttt{V3\_\_add\_close\_evidence.sql} tuvo un problema real, encontrado por
una revisión externa y reproducido aquí: el commit \texttt{0544312} (18/09) corrigió una cifra de
tamaño de foto incorrecta en un \emph{comentario} de ese archivo, sin tocar ninguna sentencia SQL
—pero Flyway calcula el \emph{checksum} de cada migración sobre el archivo completo, comentarios
incluidos, así que cualquier base que ya tuviera \texttt{V3} aplicada desde antes de ese commit
queda con un checksum registrado distinto del que Flyway recalcula al arrancar. Lo reproduje
aplicando \texttt{V1}, \texttt{V2} y la \texttt{V3} original (\texttt{b7acb32}) con Flyway 9.22.3
sobre un CockroachDB real, y corriendo después \texttt{validate}/\texttt{migrate} con la
\texttt{V3} corregida: ambos terminan con \texttt{FlywayException: Migration checksum mismatch
for migration version 3}. Ninguna migración versionada de Flyway debe editarse una vez aplicada
—el archivo no vuelve a tocarse—; \texttt{db-cluster/README.md} documenta el procedimiento de
reparación (\texttt{flyway repair}) para cualquier base persistente que ya tuviera la V3 anterior
al commit \texttt{0544312}.
